...
def get_pixel_interface(config, brightness, *args, **kwargs):
    ...
    try:
        ...
        hardware_log('LED: selecting library "rpi_ws2812x"')
    except ImportError:
        pixelModule = importlib.import_module('neopixel')
        Pixel = getattr(pixelModule, 'Adafruit_NeoPixel')
        hardware_log('LED: selecting library "neopixel" (older)')
    ...
    if led_strip_config == 'RGB':
        ...
        hardware_log('LED: disabled (Invalid LED_STRIP value: {0})'.format(led_strip_config))
        return None

    hardware_log('LED: hardware GPIO enabled, count={0}, pin={1}, freqHz={2}, dma={3}, invert={4}, chan={5}, strip={6}/{7}'.format(config['LED_COUNT'], config['LED_PIN'], config['LED_FREQ_HZ'], config['LED_DMA'], config['LED_INVERT'], config['LED_CHANNEL'], led_strip_config, led_strip))
