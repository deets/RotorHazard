    ...
    for loader, name, ispkg in pkgutil.iter_modules():
        if name.endswith('_'+type):
            try:
                ...
                hardware_log('Loaded module {0}'.format(name))
            except ImportError:
                pass
...
def discover_plugins(mod_type, *args, **kwargs):
    ...
    for plugin_module in discover_modules(mod_type):
        try:
            ...
            hardware_log('Error loading plugin {0}: {1}'.format(plugin_module.__name__, err))
            pass
    return plugins
...
class BaseHardwareInterface(object):
    ...
    def log(self, message):
        ...
        if callable(self.hardware_log_callback):
            ...
            hardware_log('Interface: {0}'.format(message))
